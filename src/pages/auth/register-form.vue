  ...
  <view class="page-auth-sub">
    <view class="page-auth-sub__main">
      ...
      <view class="auth-split-links">
        <view class="auth-split-links__item" @tap="goLogin">
          <text class="auth-split-links__muted">已有账号，去</text>
          <text class="auth-split-links__highlight">登录</text>
        </view>
      </view>
    </view>
...
async function sendCode() {
  ...
  try {
    ...
    uni.navigateTo({
      url: `/pages/auth/sms-verify?intent=register&phone=${encodeURIComponent(phone.value.trim())}`,
    })
  } catch (error) {
...
.auth-split-links__muted {
  color: $slate-500;
}

.auth-split-links__highlight {
  color: $primary;
  font-weight: 600;
}
