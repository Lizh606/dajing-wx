...
async function loadServiceList() {
  ...
  try {
    ...
    if (serviceLoadToken.value === token) {
      showFailToast(getErrorMessage(error, '检测项目加载失败'))
    }
  } finally {
    if (serviceLoadToken.value === token) {
...
async function loadServiceByCategory(category: string) {
  ...
  try {
    ...
    if (serviceLoadToken.value === token) {
      showFailToast(getErrorMessage(error, '分类检测项目加载失败'))
      services.value = []
    }
  } finally {
    if (serviceLoadToken.value === token) {
...
.service-card__org {
  @include service-card-org(20rpx, #64748b);
}
...
.service-card__tags {
  @include service-card-tags(null, null, 6rpx);
}
